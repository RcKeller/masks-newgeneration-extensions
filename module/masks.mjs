...
Hooks.once("init", () => {
    ...
    // game.settings.register("masks-newgeneration-extensions", "enable_dark_mode", {
    ...
    if (game.settings.get("masks-newgeneration-extensions","enable_dark_mode")){
		var link = document.createElement('link');
		link.rel = 'stylesheet';
		link.type = 'text/css';
		link.href = './modules/masks-newgeneration-extensions/css/dark-mode.css';
		//Append link element to HTML head
		head.appendChild(link);
	}

    // Register settings
    game.settings.register('masks-newgeneration-extensions', 'firstTime', {
...
Hooks.once('ready', async function () {
    ...
    if (game.settings.get('masks-newgeneration-extensions', 'firstTime')) {
        game.settings.set('masks-newgeneration-extensions', 'firstTime', false);

        const callback = async () => {
            game.settings.set('masks-newgeneration-extensions', 'firstTime', true);
            const worldData = {
                id: game.world.id,
                action: 'editWorld',
                background: `modules/masks-newgeneration-extensions/images/login-bg-lt.webp`,
            };
            let response;
            try {
        ...
        if (game.settings.settings.has('masks-newgeneration-extensions.enableLoginImg')) {
            if (game.settings.get('masks-newgeneration-extensions', 'enableLoginImg')) {
                const worldData = {
                    id: game.world.id,
                    action: 'editWorld',
                    background: `modules/masks-newgeneration-extensions/images/login-bg-lt.webp`,
...
Hooks.on("preCreateActor", async function (document) {
    if (document.type === 'character') {
        document.updateSource({'flags.masks-newgeneration-extensions.influences': []});
    }
});
...
function onInfluenceCreate(actor, html) {
    ...
    create.addEventListener('click', async function(event) {
        ...
        let influences = actor.getFlag("masks-newgeneration-extensions", "influences") ?? [];
        influences.push(item);
        actor.setFlag("masks-newgeneration-extensions", "influences", influences);
    });
}
...
function onInfluenceAction(actor, html) {
    ...
    actions.forEach(action => {
        ...
        await actor.setFlag("masks-newgeneration-extensions", "influences", influences);
        });
    });
}
    ...
    const MODULE_CONFIG = {
        ...
        buttonsData: [
            ...
            },
            {
                action: (ev) => {
                    ev.preventDefault();
                    window.open("https://github.com/RcKeller/masks-newgeneration-extensions", "_blank");
                },
